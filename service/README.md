# InvoiceGuard service

The integration-grade engine: a FastAPI service over the same five rule layers
the browser console runs, plus the supplier baseline and the payee-account
ledger that only make sense server-side.

For the project overview and the console, see the [top-level README](../README.md).
...
baseline and starts the server.
